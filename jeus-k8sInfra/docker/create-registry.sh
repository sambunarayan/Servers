#!/usr/bin/env bash
certs=/etc/docker/certs.d/192.168.2.10:8443
...
for i in {1..3}
  do
    sshpass -p vagrant ssh -o StrictHostKeyChecking=no root@192.168.2.10$i mkdir -p $certs
    sshpass -p vagrant scp tls.crt 192.168.2.10$i:$certs
  done
